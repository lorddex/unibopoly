...
class m3_kp_api():
	...
	def load_subscribe_sparql(self, query_str, handler):		
		...
		self.first_result = self.test_sparql.subscribe_sparql(query_str, handler)
	...
	def load_query_sparql (self, query_sparql_in):
		...
		return 	self.result_sparql_query


	############################### Sparql Update == Sparql Query ##################

	def load_update_sparql (self, query_sparql_in, scheduled_time = None):
		
		if scheduled_time != None:
			query_sparql_in = "PREFIX scheduled_time: <%s> %s" % (str(scheduled_time), query_sparql_in)
