...
#Utilities:
#context_update_function
def update_sparql_context (context, added, removed):
    updated_context=[]

    if removed != None:
	    for result in context:
		to_remove = False
		for result_r in removed:	
			if result_r == result:
				to_remove = True

		if to_remove == False:
			updated_context.append(result)


    if added != None:
	    for result_a in added:	
		to_add = True
		for result in updated_context:
			if result_a == result:
				to_add = False

		if to_add == True:
			updated_context.append(result_a)
...
class RDFSubscribeHandler(threading.Thread):
	...
	try :
		self.msg_handler.handle(first_result,[])
	except:
		None
...
class sparqlSubscribeHandler(threading.Thread):
	...
	try :
		...
		for el in first_result:
			x=el
			x.sort()
			first_res_sort.append(x)

		
		#first_result.sort()

		self.msg_handler.handle(first_res_sort, [])
		#self.msg_handler.handle(first_result, [])
	except:
		None
    ...
    def run(self):
        ...
        while True:
		    ...
		    for el in added_list:
			 x=el
			 x.sort()
   			 added_list_sort.append(x)

		    removed_list_sort=[]
		    for el in removed_list:
			 x=el
			 x.sort()
   			 removed_list_sort.append(x)
	
		    #added_list.sort()
	 	    #removed_list.sort()

                    self.msg_handler.handle(added_list_sort, removed_list_sort)
		    #self.msg_handler.handle(added_list, removed_list)
